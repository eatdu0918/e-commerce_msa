...
export default function ReviewList({ productId }: ReviewListProps) {
    ...
    if (isLoading) return <div className="text-center py-10">{t('reviewList.loading')}</div>;

    if (reviews.length === 0) {
        return (
            <div className="text-center py-12 bg-stone-50 rounded-2xl border border-stone-100">
                <p className="text-stone-400">{t('reviewList.empty')}</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <h3 className="text-xl font-bold">{t('reviewList.heading', { count: reviewsPage?.totalElements ?? 0 })}</h3>
            <div className="space-y-6">
                {reviews.map((review) => (
                    <div key={review.id} className="border-b border-stone-100 pb-6 last:border-0">
                        <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center space-x-2">
                                ...
                                <div>
                                    <p className="text-sm font-bold">{review.userName || t('common.anonymous')}</p>
